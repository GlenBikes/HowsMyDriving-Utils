const date_ext = require('../src/util/extend/date-ext');
const str_utils = require('../src/util/string_utils');
...
function requireLocale(locale) {
  try {
    logging.log.debug(`requiring locale ${locale}...`);
    var normpath = path.normalize('../locales/' + locale + '.js');

    logging.log.debug(`Loading locale ${locale} from ${normpath}.`);

    require(normpath);
  } catch (error) {
    throw Error(
      'Failed to load the following locale: ' + locale + `error: ${error}`
    );
  }
}
...
function pluralize(unit, value, config) {
  ...
  var unitString = numerous.pluralize(config.locale, value, unitTypeData[unit]);
...
function getLocaleDataForUnitType(config) {
  ...
  if (null === unitTypeData) {
    throw new Error(
      'Can not find any unit type data for locale: ' + config.locale
    );
  }
...
function getDifference(firstDate, secondDate) {
  ...
  if (difference > 1000) {
    ...
    // The difference is less than the smallest unit. Return 1 second.
    results.push([timeUnits[timeUnits.length - 1][0], 1]);
  }
